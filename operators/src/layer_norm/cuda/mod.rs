use std::{ffi::CString, sync::{Arc, Mutex}, usize};
...
use crate::{
    cuda::{dt_name, Gpu, Handle, ModuleBox}, get_static, layer_norm::args::Meta, shape_not_support, strides_not_support, ByteOf, LaunchError, QueueAlloc, SchemeDiversity, SchemeError
};
...
impl crate::Operator for Operator {
        ...
        QA: QueueAlloc<Hardware = Self::Hardware>,
    {
        ...
        if dsy!= unit || dsx != unit || dss != dt_w.nbytes() as isize || dsb != dt_w.nbytes() as isize  {
...
extern "C" __global__ void {name}(
    ...
    (y, stride_y, x, stride_x, s,b, epsilon);
            ...
            let name = format!("layer_norm_{ta}_{tw}_folding_{num_threads_block}x{num_items_thread}");
            let module = handle.compile_kernel(&name, cc, || {
                format!(
                    r#"{CODE}
...
mod test {
    ...
    use digit_layout::{types::{F16, F32, F64}, DigitLayout};
